...
| `test_agent_evals.py`               | Deterministic prompt-shaped workflow evaluations with structural scoring                                 |
| `test_e2e_client_server.py`         | Real subprocess server with stdio transport, server log capture                                          |
| `test_streamable_http_transport.py` | Real Uvicorn server with live synchronous and asynchronous HTTP clients                                  |
...
`test_agent_evals.py` exercises representative agent intents without depending
on a probabilistic model. Each case defines an expected tool sequence and a
maximum call budget, then scores protocol responses for tool availability,
error status, structured content, and required result paths. The suite covers
schema discovery, bounded aggregation, recovery from a rejected write, and
query-plan inspection.

These evaluations catch contract regressions while remaining deterministic in
CI. They complement, rather than replace, model-based quality evaluations.
...
│   ├── test_agent_evals.py           # Deterministic agent workflow evaluations
│   ├── test_e2e_client_server.py     # Subprocess server E2E
│   ├── test_streamable_http_transport.py # Live HTTP transport E2E
